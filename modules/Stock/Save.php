...
// Do not permit same product twice in the same warehouse
// This verification should be made in before_save event to catch all cases
if ($mode!='edit') {  // creating
	$cnt=$adb->getone('select count(*) from vtiger_stock
		where whid='.$focus->column_fields['whid'].
		' and pdoid='.$focus->column_fields['pdoid']);
	if ($cnt>0) {
		die("<br><br><center>".getTranslatedString('CannotCreate','Stock')." <a href='index.php?module=Stock&action=ListView'>".getTranslatedString('LBL_GO_BACK').".</a></center>");
	}
} 
